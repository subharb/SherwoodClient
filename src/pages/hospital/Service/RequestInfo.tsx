        ...
        for(let i = 0; i < allRequests.length; i++){
            if(allRequests[i].length > 0){
                ...
                const requests = request.requestsServiceInvestigation.map((reqSer) =>{
                    ...
                    return(<ChipContainer><ColourChip  rgbcolor={statusToColor(reqSer.status)}>{translate(`pages.hospital.services.tests.${typeService}.${serviceCode}`)}</ColourChip></ChipContainer>)})
