...
export const saveListPatients = async (patients: IPatient[], investigation: any): Promise<void> => {
    ...
    const patientsToSave = patients.map((patient, id) => {
        ...
        patient.personalData = patient.personalData ? decryptSinglePatientData(patient.personalData, keyInvestigation, investigation.permissions, investigation.personalFields) : null;
        return {
            id: patient.id,
            uuid: patient.uuid,
