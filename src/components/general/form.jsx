...
import {
  ...
  ButtonContinue,
  ButtonDelete,
} from "../../components/general/mini_components";
...
class Form extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      this.props.fields[changedField] &&
      this.props.fields[changedField].callBackOnChange
    ) {
      this.props.fields[changedField].callBackOnChange(
        this.props.formValues[changedField]
      );
    } else if (this.props.fields[changedField]) {
      const value = getValueField(
        this.props.fields[changedField],
        this.props.formValues[changedField]
      );
  ...
  renderExtraFields(key) {
    ...
    if (
      activatedFields &&
      activationValues &&
      activationValues.includes(value)
    ) {
      ...
      if (this.props.fields[key].type !== "options") {
        ...
        currentSection.push(
          <Grid
            item
            xs={
              this.props.fields[key].numberColumnsXs
                ? this.props.fields[key].numberColumnsXs
                : 12
            }
            lg={
              this.props.fields[key].numberColumnsLg
                ? this.props.fields[key].numberColumnsLg
                : 12
            }
          ...
          >
            <Field
              name={
                this.props.fields[key].name ? this.props.fields[key].name : key
              }
        ...
        if (index === Object.keys(this.props.fields).length - 1) {
          fieldsMarkup.push(
            <Paper elevation={3} style={{ padding: "1rem", marginTop: "1rem" }}>
              <Grid container>{currentSection}</Grid>
            </Paper>
          );
        }
      }
          ...
          <div style={{ paddingTop: "1rem" }}>
            ...
            <ButtonContinue
              ...
              onClick={this.props.handleSubmit((values) => {
                this.callBackForm(values, "button1");
              })}
            >
              {this.props.submitText
                ? this.props.translate(this.props.submitText)
            ...
            {this.props.alterSubmitButton && (
              <ButtonContinue
                ...
                spaceright={1}
                onClick={this.props.handleSubmit((values) => {
                  this.callBackForm(values, "button2");
                })}
              >
            ...
            {this.props.closeCallBack && (
              ...
              >
                {this.props.cancelText
                  ? this.props.translate(this.props.cancelText)
...
function validate(values, props) {
  ...
  Object.keys(dictFields).forEach((key) => {
    ...
    const fieldValueCompare = dictFields[key].validationField
      ? values[dictFields[key].validationField]
      : dictFields[key].validationValue
      ? props.translate(dictFields[key].validationValue)
      : null;
    const valueField =
      dictFields[key].type === "textarea" && typeof values[key] !== "undefined"
        ? values[key].replace(/<[^>]+>/g, "")
        : values[key];
    const validationFunc = dictFields[key].validation
      ? dictFields[key].validation
      : "notEmpty";
    const validation = validateField(
      {
        value: valueField,
